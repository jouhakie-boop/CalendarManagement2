...
public class ConflictDetector
{
    ...
    public ConflictDetector(CalendarRepository repo) => _repo = repo;

    // Returns list of conflicting pairs
    public List<(Event a, Event b)> FindConflicts()
    {
        var conflicts = new List<(Event, Event)>();
        var events = _repo.GetAllEvents(); // add this method if missing
    ...
    bool Overlaps(Event x, Event y)
    {
        if (!TryGetStartEnd(x, out var sx, out var ex)) return false;
        if (!TryGetStartEnd(y, out var sy, out var ey)) return false;
        return sx < ey && sy < ex;
    }

    // Parse using current model's Date and Time strings; defaults to 1-hour duration if no end time
    bool TryGetStartEnd(Event e, out DateTime start, out DateTime end)
    {
        start = DateTime.MinValue;
        end = DateTime.MinValue;
        if (!DateTime.TryParse($"{e.Date} {e.Time}", out start)) return false;
        // If you add EndTime or Duration to Event, parse that here instead.
